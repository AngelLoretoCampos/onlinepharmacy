...
<head>
    ...
    <style>
        .product-card {
            width: 250px;
            height: 350px; /* Adjusted height to accommodate longer product names */
        }

        .product-card img {
            max-width: 100%;
            max-height: 100%;
            object-fit: cover; /* Changed object-fit property */
        }
            ...
            while($row = $result->fetch_assoc()) {
                ...
                <a href="view_product.php?id=' . $product_id . '" class="hover:underline">
                    <div class="bg-white p-4 rounded-lg shadow-md product-card">
                        <img src="data:image/jpeg;base64,' . $product_image . '" alt="' . $product_name . '" class="w-full h-48 mb-4">
